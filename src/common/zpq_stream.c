...
static ssize_t zpq_write_compressed(ZpqStream * zc, char const *src, size_t src_size, size_t *src_processed){
...
static void zpq_write_raw(ZpqStream * zc, char const *src, size_t src_size, size_t *src_processed) {
    ...
    zc->tx_total_raw += copy_len;
    zc->tx_total += copy_len;

    zc->tx_size += copy_len;
    *src_processed = copy_len;
}

ssize_t zpq_write(ZpqStream * zq, void const *buf, size_t size, size_t *processed) {
    size_t		buf_pos = 0;
    ssize_t		rc;
    ...
    do
    {
        ...
        while (!(ZPQ_BUFFER_SIZE - zq->tx_size > 10 && (size - buf_pos > 0 || zs_buffered_tx(zq->zs))) && zq->tx_pos < zq->tx_size)
        {
        ...
        if (zq->tx_pos == zq->tx_size) {
            zq->tx_pos = zq->tx_size = 0; /* Reset pointer to the beginning of buffer */
        }
        ...
        while ((zq->tx_msg_bytes_left > 0) && ((size - buf_pos) >= (zq->tx_msg_bytes_left + 5 - zq->tx_msg_h_size))) {
            char msg_type;
            msg_type = *((char*)buf + buf_pos + zq->tx_msg_bytes_left - zq->tx_msg_h_size);
            uint32 msg_len;
        ...
        if (zs_buffered_tx(zq->zs) || (zq->is_compressing && zq->tx_msg_bytes_left > 0)) {
            ...
            if (zq->tx_msg_h_size > 0) {
                ...
                if (zq->tx_msg_bytes_left < (size - buf_pos)) {
                    to_compress = zq->tx_msg_bytes_left;
                } else {
        ...
        } else if (zq->tx_msg_bytes_left == 0) { /* determine next message type */
            /* try to get next msg type, then set is_compressing and tx_msg_bytes_left */
            if (zq->tx_msg_h_size == 5) { /* read msg type and length if possible */
                char msg_type;
                msg_type = *((char*)zq->tx_msg_h_buf);
                uint32 msg_len;
                ...
                if (zpq_should_compress(msg_type, msg_len)) {
                    ...
                    if (zq->is_compressing) { /* may return to this multiple times */
                        while (zs_buffered_tx(zq->zs)) {
                ...
                if (avail > (5 - zq->tx_msg_h_size)) {
                    avail = (5 - zq->tx_msg_h_size);
                }
                memcpy((char*)zq->tx_msg_h_buf + zq->tx_msg_h_size, (char*)buf + buf_pos, avail);
                zq->tx_msg_h_size += avail;
                buf_pos += avail;
            }
        } else {
            size_t copy_size = zq->tx_msg_bytes_left;
            size_t copy_rc = 0;
            if (zq->tx_msg_h_size > 0)
            {
                if (zq->tx_msg_h_size < copy_size) {
                    copy_size = zq->tx_msg_h_size;
                }
                zpq_write_raw(zq, (char *) zq->tx_msg_h_buf + 5 - zq->tx_msg_h_size, copy_size, &copy_rc);
                zq->tx_msg_h_size -= copy_rc;
            } else {
                if ((size - buf_pos) < copy_size) {
                    copy_size = size - buf_pos;
                }
                zpq_write_raw(zq, (char *) buf + buf_pos, copy_size, &copy_rc);
                buf_pos += copy_rc;
            }
            zq->tx_msg_bytes_left -= copy_rc;
        }
        /*
...
ssize_t zpq_read(ZpqStream * zc, void *buf, size_t size) {
    size_t		buf_pos = 0;
    size_t		rx_processed;
...
size_t zpq_buffered_rx(ZpqStream * zc) {
...
size_t zpq_buffered_tx(ZpqStream * zc) {
