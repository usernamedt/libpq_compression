...
zpq_write(ZpqStream * zq, void const *buf, size_t size, size_t *processed)
{
	...
	do
	{
		...
		/*
		 * try to prefetch the next message types and increase
		...
		{						/* determine next message type */
			...
			{					/* read msg type and length if possible */
				...
				{
					/*
					 * random message type for test (forbid compressing non-d
					 * messages)
					 */
					if (zq->is_compressing)
					{			/* may return to this multiple times */
